...
\begin{analysis}
左边是一些绝对值的和式, 不便于直接放缩. 再考虑到所有的变量均地位相等, 故可以考虑设序: 不妨设$ x_1\ge x_2\ge\cdots\ge x_n $. 这样, 左边的绝对值就可以去掉了, 变成了一个普通的和式:
\[ \sum_{1 \leq i < j \leq n} |x_i - x_j| = \sum_{1 \leq i < j \leq n} (x_i - x_j) = \sum_{i=1}^n (n + 1 - 2i) x_i. \]\par
注意到这是各$ x_i $的线性组合, 而每个$ x_i $都有自己的取值范围, 故可以直接将系数为正的$ x_i $放缩到$ 1 $, 将系数为负的$ x_i $放缩到$ 0 $, 即可得到最大值. \par
最后检查取等条件: 
\[ x_i=\begin{cases}
1,&1\le i\le \fl{\frac{n+1}{2}};\\
0,&\fl{\frac{n+1}{2}}<i\le n.
\end{cases} \] 
发现可以取等, 故我们完成了证明.
...
\begin{example}
设正整数$ a_1, a_2, \cdots, a_{31}, b_1, b_2, \cdots, b_{31} $满足$ 1\le a_1<a_2<\cdots<a_{31}\le 2015 $, $ 1\le b_1<b_2<\cdots<b_{31}\le 2015 $, 且$ \sum_{i=1}^{31}a_i=\sum_{i=1}^{31}b_i $. 求
\[ \sum_{i=1}^{31}\left|a_i-b_i\right| \]
的最大值.
\end{example}

\begin{analysis}
先来看一下取等. 直观上, 我们希望$ \{a_i\} $和$ \{b_i\} $一个较为分散, 而另一个较为紧密. 可以取
\[ a_1=1, a_2=2, \cdots, a_{15}=15, a_{16}=2000, a_{17}=2001,\cdots,a_{31}=2015, \]
而$ \{b_i\} $取为等差数列. 此时不难计算出原式不大于$ 30720 $.\par
首先, 来观察一下$ \sum_{i=1}^{31}\left|a_i-b_i\right| $的形式. 绝对值并不利于直接放缩, 并且难以利用条件 $ \sum_{i=1}^{n}a_i=\sum_{i=1}^{n}b_i $. 因此考虑去掉绝对值. 记$ A= \{1\le i\le 31:\ a_i\ge b_i\}$, $ B=\{1\le i\le 31:\ a_i<b_i\} $. 那么原式可以写为
\[ S:=\sum_{i=1}^{31}\left|a_i-b_i\right|=\sum_{i\in A} (a_i-b_i)+\sum_{i\in B} (b_i-a_i). \]\par
由$ \sum_{i=1}^{n}a_i=\sum_{i=1}^{n}b_i $知
\[ S=2\sum_{i\in A} (a_i-b_i)=2\sum_{i\in B}(b_i-a_i). \]
继续考虑上面的式子的形式. 条件$ 1\le a_1<a_2<\cdots<a_{31}\le 2015 $与$ 1\le b_1<b_2<\cdots<b_{31}\le 2015 $实际上是比较宽松的条件. 根据取等, 对$ i\in A, j\in B $, $ a_i-b_i $的均值与$ b_j-a_j $的均值比较接近且两者之和大约为$ 2015 $, $ |A| $与$ |B| $比较接近, $ a_i-b_i\ (i\in A) $均相等. 这些都意味着我们只需要使用如下平凡的放缩: 将$ \sum_{i\in A}(a_i-b_i) $与$ \sum_{i\in B}(b_i-a_i) $通过某种方式加起来就可以达到效果.\par 
因此, 我们找$ \lambda,\mu $, 最小化
\[ \lambda\sum_{i\in A}(a_i-b_i)+\mu\sum_{i\in B}(b_i-a_i)=(\lambda+\mu)\cdot 2S \] 
即可. $ \sum_{i\in A}(a_i-b_i) $与$ \sum_{i\in B}(b_i-a_i) $的地位基本相同, 而由取等可以看出来$ a_i-b_i\ (i\in A) $与$ b_i-a_i\ (i\in B) $各自的均值较为重要, 因此可以通过取$ \lambda=1/|A|,\mu=1/|B| $来表示这两个均值:
\[ \frac{1}{|A|}\sum_{i\in A}(a_i-b_i)+\frac{1}{|B|}\sum_{i\in B}(b_i-a_i)=(\frac{1}{|A|}+\frac{1}{|B|})\cdot S. \]\par
两边同时乘上$ |A|\cdot |B| $, 得到
\begin{align*}
2\cdot 31S&=|B|\sum_{i\in A}(a_i-b_i)+|A|\sum_{i\in B}(b_i-a_i)\\
&=\sum_{i\in A,j\in B}(a_i-b_i)+\sum_{i\in A,j\in B}(b_j-a_j)=\sum_{i\in A,j\in B}(a_i-b_i+b_j-a_j).
\end{align*}\par
因此只需要观察$ a_i-b_i+b_j-a_j $有没有较好的放缩. \par
以$ i<j $为例. $ a_i-a_j\le i-j $, 而$ b_j\le 1984+j $, $ b_i\ge i $, 因此
\[ a_i-b_i+b_j-a_j\le (i-j)+(1984+j)-i=1984. \]
这样, 正好符合取等中$ a_i-b_i\ (i\in A) $全相等, 且$ b_i-a_i\ (i\in B) $全相等. 由此可以计算出$ S\le 30720. $\par


\end{analysis}

\begin{solution}
记$ A= \{1\le i\le 31:\ a_i\ge b_i\}$, $ B=\{1\le i\le 31:\ a_i<b_i\} $, $ S_1=\sum_{i\in A} |a_i-b_i|$. $ S_2=\sum_{i\in B} |a_i-b_i| $. 则原式为$ S_1+S_2 $. \par
由$ \sum_{i=1}^{31}a_i=\sum_{i=1}^{31}b_i $知$ \sum_{i\in A} (a_i-b_i)=\sum_{i\in B}(b_i-a_i)$, 即$ S_1=S_2$. \par
由平均值原理, 知存在$ i\in A,j\in B $(不妨设$ i>j $), 使得
\[ a_i-b_i\le \frac{S_1}{|A|},\quad b_j-a_j\le \frac{S_2}{|B|}. \] \par
因此,
\[ \frac{S_1}{|A|}+\frac{S_2}{|B|}\le(a_i-a_j)-(b_i-b_j)\le (2015-31+i-j)-(i-j)=1984. \] \par
由$ S_1=S_2 $知
\[ S_1=S_2\le \frac{1984|A||B|}{|A|+|B|}\le \frac{1984\cdot15\cdot16}{31}=15360. \] \par
因此, 原式$ S_1+S_2\le 30720 $. 
\end{solution}


\begin{example}
给定正整数$ n $. 对$ a_k\in [0,1]\ (1\le k\le n) $, 记
\[ A_k=\frac{1}{k}\sum_{i=1}^{k}a_i. \] 
求证:
\[ \left|\sum_{k=1}^{n}(A_k-a_k)\right|\le\ce{\frac{n}{\mathrm{e}}}. \] 
\end{example}

\begin{analysis}
注意到各$ A_k $也是各$ a_i $的线性组合, 因此, 左边本质上也是各$ a_i $的线性组合:
\[ \sum_{k=1}^{n}(A_k-a_k)=\sum_{k=1}^{n}\left[\frac{1}{k}\left(\sum_{i=1}^{k}a_i\right)-a_k\right]=\sum_{i=1}^{n}\left(\sum_{k=i}^{n}\frac{1}{k}-1\right)a_i. \]\par
先考虑这个式子的上界. 与例题3.1.4基本相同, 只需要把系数为正的$ a_i $放缩为$ 1 $, 系数为负的$ a_i $放缩为$ 0 $即可. 因此, 我们只需要考虑各个系数$ \sum_{k=i}^{n}\frac{1}{k}-1 $的正负性. 注意到$ \sum_{k=i}^{n}\frac{1}{k} $是调和级数的一部分, 其近似为$ \ln\frac{n}{i-1} $(这也解释了右边系数$ \mathrm{e}^{-1} $的由来). 因此有如下粗略的估计:
\[ \sum_{k=i}^{n}\frac{1}{k}-1\approx \ln\frac{n}{i-1}-1\begin{cases}
>0,&i<1+n/\mathrm{e};\\
<0,&i>1+n/\mathrm{e}.
\end{cases} \] \par
因此, 我们就知道, 大约当$ a_i=1\ (i\le \ce{n/\mathrm{e}}),a_i=0\ (i>\ce{n/\mathrm{e}}) $时, 取到最大值. \par
对于下界, 可以做类似的放缩. 计算可得, 绝对值的最大值确实为$ \ce{n/\mathrm{e}} $. \par
\end{analysis}

\begin{solution}
\[ \left|\sum_{k=1}^{n}(A_k-a_k)\right|=\left|\sum_{k=1}^{n}\left[\frac{1}{k}\left(\sum_{i=1}^{k}a_i\right)-a_k\right]\right|=\left|\sum_{i=1}^{n}\left(\sum_{k=i}^{n}\frac{1}{k}-1\right)a_i\right|. \]
设$ r $为使得$ \sum_{j=k}^{n}1/j -1>0 $的最大的$ k $. 则
\[ \sum_{k=1}^{n}a_k\left(1-\sum_{j=k}^{n}\frac{1}{j}\right)\le\sum_{k=r+1}^{n}a_k\left(1-\sum_{j=k}^{n}\frac{1}{j}\right)\le\sum_{k=r+1}^{n}\left(1-\sum_{j=k}^{n}\frac{1}{j}\right); \]
\[ \sum_{k=1}^{n}a_k\left(\sum_{j=k}^{n}\frac{1}{j}-1\right)\le\sum_{k=1}^{r}a_k\left(\sum_{j=k}^{n}\frac{1}{j}-1\right)\le\sum_{k=1}^{r}\left(\sum_{j=k}^{n}\frac{1}{j}-1\right). \]\par
由于
\[ \sum_{k=1}^{n}\left(1-\sum_{j=k}^{n}\frac{1}{j}\right)=n-\sum_{k=1}^{n}\sum_{j=k}^{n}\frac{1}{j}=n-\sum_{j=1}^{n}\left(\frac{1}{j}\sum_{k=1}^{j}1\right)=n-n=0, \] 
故有$ \sum_{k=1}^{r}\left(\sum_{j=k}^{n}1/j-1\right)=\sum_{k=r+1}^{n}\left(1-\sum_{j=k}^{n}1/j\right). $ \par
接下来我们估计上面的式子. 
\begin{align*}
\sum_{k=r+1}^{n}\left(1-\sum_{j=k}^{n}\frac{1}{j}\right)&=(n-r)-\sum_{k=r+1}^{n}\sum_{j=k}^{n}\frac{1}{j}=(n-r)-\sum_{j=r+1}^{n}\frac{j-r}{j}\\
&=(n-r)-(n-r)+\sum_{j=r+1}^{n}\frac{r}{j}=r\sum_{j=r+1}^{n}\frac{1}{j}<r.
\end{align*}
再由于$ \sum_{j=r}^{n}1/j\ge 1 $, 故\note{下面就是利用$ r $的定义来对$ r $进行估计}
\[ 1\le \sum_{j=r}^{n}\frac{1}{j}\le \sum_{j=r}^{n}\ln\frac{j}{j-1}=\ln\frac{n}{r-1}. \]
由此知道$ r-1\le n/\mathrm{e} $, 故$ r\le\ce{n/\mathrm{e}} $.\par
因此,
\[ \left|\sum_{k=1}^{n}(A_k-a_k)\right|\le r\le\ce{\frac{n}{\mathrm{e}}}. \] 
\end{solution}

\textbf{注}\ \ 进一步, 系数$ 1/\mathrm{e} $是最优的. 取
\[ a_1=a_2=\cdots=a_r=0,\ a_{r+1}=\cdots=a_n=1, \]
通过计算可知
\[ \left|\sum_{k=1}^{n}(A_k-a_k)\right|=\sum_{k=r+1}^{n}\left(1-\sum_{j=k}^{n}\frac{1}{j}\right)\ge\ce{\frac{n+1}{\mathrm{e}}}-2. \]  
