...
\[
\sum_{k=1}^n \left(x\left[\frac{k}{x}\right]-(x+1)\left[\frac{k}{x+1}\right]\right)=\sum_{k=1}^n (xq_k-(x+1)Q_k)=\sum_{k=1}^n (R_k-r_k),
\]
...
这由
\[
\sum_{1\le i<j \le n}[a_ia_j]\ge \sum_{k=2}^n [a_1a_k]\ge [a_1(n-a_1)]-n+2\ge 2(n-2)-n+2=n-2
\]
...
那么
\[
\sum_{1\le i<j\le n}[a_ia_j]\ge |A|\ge n-2.
\]
...
\[
\sum_{1\le i,j\le n}[a_ib_j]\ge n-1.
\]

若存在$a_i\ge 2$或$b_i\ge 2$, 不妨设$a_1\ge 2$, 则有: 
\[
\sum_{1\le i,j\le n}[a_ib_j]\ge \sum_{k=1}^n [a_1b_k]\ge [na_1]-n+1>n-1,
\]
